...
while($operation){
    ...
    if ((-not ($choice -match "^[1-9]$|^10$"))){Write-Host "Not a valid input."; continue;}
    ...
    elseif($choice -eq 7){
        ...
        # TODO: Check the given username with the checkUser function.
        if (-not (checkUser $name)){Write-Host "User does not exist."; continue;}
    ...
    elseif($choice -eq 9){
        ...
        Write-Host ($userLogins | Group-Object User | where-object {$_.count -ge 10} | select count, name | Format-Table | Out-String)
